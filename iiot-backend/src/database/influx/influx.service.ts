...
export class InfluxService implements OnModuleInit {
  ...
  getQueryApi() {
    ...
    return this.influxDB.getQueryApi(
      this.configService.get<string>('INFLUX_ORG', 'ChestaCorp'),
    );
  }
  ...
  onModuleInit() {
    ...
    if (!this.enabled) {
      this.logger.warn(
        'InfluxDB integration disabled via INFLUX_ENABLED=false',
      );
      return;
    }
    ...
    const url =
      this.configService.get<string>('INFLUX_URL') || 'http://influxdb:8086';
    const token = this.configService.get<string>('INFLUX_TOKEN');
    const org = this.configService.get<string>('INFLUX_ORG') || 'ChestaCorp';
    const bucket =
      this.configService.get<string>('INFLUX_BUCKET') || 'iiot_raw_data';
