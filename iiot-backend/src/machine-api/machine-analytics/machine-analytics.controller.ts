...
export class MachineAnalyticsController {
  ...
  @ApiOperation({
    summary: 'Ringkasan performa seluruh line produksi (Live + Stats)',
  })
  @ApiResponse({
    status: 200,
    description: 'Summary data berhasil dikompilasi',
  })
      ...
      analytics: {
        ...
        totalDowntimeMinutes:
          todayStats.data?.reduce(
            (acc: number, m: any) => acc + (m.totalDowntimeMinutes || 0),
            0,
          ) || 0,
      },
    };
  ...
  @Roles(UserRole.OPERATOR, UserRole.SUPERVISOR, UserRole.MANAGER)
  @ApiOperation({
    summary: 'Mendapatkan trend produksi/telemetri dari InfluxDB',
  })
  ...
  @ApiQuery({
    ...
    required: false,
    example: '-1h',
    description: 'Range waktu (e.g. -1h, -24h, -7d)',
  })
  async getTrend(
    ...
    return await this.influxAnalyticsService.getProductionTrend(
      machineId,
      range,
      window,
    );
