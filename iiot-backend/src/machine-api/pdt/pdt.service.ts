...
export class PdtService {
  ...
  async unlockAndUpdatePdt(id: number, dto: UpdatePdtDto, username: string) {
    ...
    const isValid = await this.authService.validatePassword(username, dto.password);
    if (!isValid) {
      throw new UnauthorizedException('Password salah. Gagal membuka kunci data PDT!');
    }

    // 2. Cek apakah record PDT ada
    const record = await this.prisma.plannedDowntime.findUnique({ where: { id } });
    if (!record) {
