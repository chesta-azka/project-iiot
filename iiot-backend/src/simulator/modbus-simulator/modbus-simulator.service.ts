...
class IndustrialMachine {
      ...
      constructor(id: string, name: string, speed: number, fail: number, baseTemp: number) {
            this.id = id;
            this.name = name;
...
export class ModbusSimulatorService implements OnModuleInit, OnModuleDestroy {
      ...
      private initializeFactory() {
            const config = [
                  { id: 'AQ-BLW-01', name: 'Blower Alpha', speed: 800, fail: 0.0001, t: 45 },
                  { id: 'AQ-BLW-02', name: 'Blower Beta', speed: 820, fail: 0.0001, t: 46 },
                  { id: 'AQ-FIL-01', name: 'Filler High-Speed', speed: 500, fail: 0.0002, t: 38 },
                  { id: 'AQ-FIL-02', name: 'Filler Medium', speed: 900, fail: 0.0001, t: 35 },
                  { id: 'AQ-CAP-01', name: 'Capper Rotary', speed: 600, fail: 0.0003, t: 40 },
                  { id: 'AQ-CAP-02', name: 'Capper Linear', speed: 1200, fail: 0.0001, t: 32 },
                  { id: 'AQ-LBL-01', name: 'Labeler Front', speed: 1000, fail: 0.0005, t: 50 },
                  { id: 'AQ-LBL-02', name: 'Labeler Back', speed: 1000, fail: 0.0005, t: 50 },
                  { id: 'AQ-INK-01', name: 'Inkjet Coder 1', speed: 400, fail: 0.00005, t: 30 },
                  { id: 'AQ-INK-02', name: 'Inkjet Coder 2', speed: 400, fail: 0.00005, t: 30 },
                  { id: 'AQ-PCK-01', name: 'Case Packer A', speed: 3000, fail: 0.001, t: 55 },
                  { id: 'AQ-PCK-02', name: 'Case Packer B', speed: 3200, fail: 0.001, t: 55 },
                  { id: 'AQ-PLT-01', name: 'Palletizer Robot', speed: 8000, fail: 0.002, t: 60 },
                  { id: 'AQ-WRP-01', name: 'Wrapper 1', speed: 10000, fail: 0.0008, t: 42 },
                  { id: 'AQ-WRP-02', name: 'Wrapper 2', speed: 11000, fail: 0.0008, t: 42 },
                  { id: 'AQ-CON-01', name: 'Main Conveyor', speed: 200, fail: 0.00001, t: 28 },
            ];

            this.machines = config.map(c => new IndustrialMachine(c.id, c.name, c.speed, c.fail, c.t));
      }
                  ...
                  .subscribe(async () => {
                        try {
                              ...
                              const reportTasks = this.machines.map(async (m) => {
                                    ...
                                    return this.engine.processNewData(m.id, m.name, data);
                              });
                              ...
                              this.logger.error(`[Simulation Loop] Gagal memproses batch data: ${err.message}`);
                        }
                  });
      }
      ...
      private startModbusServer() {
            try {
                  ...
                  this.modbusServer.listen(this.PORT, () => {
                        ...
                        this.logger.log(`📦 Allocated Holding Buffer: ${holdingBuffer.length} bytes`);
                  });
      ...
      private mapDataToRegisters() {
            ...
            this.machines.forEach((m: any, index) => {
                  ...
                  try {
                        ...
                        const canFail = m.status === 1 && !m.isRecovering && now > m.nextAllowedError;
                        ...
                        if (canFail && triggerError) {
                              ...
                              this.logger.warn(`🚨 [FAILURE] ${m.name || 'Unit-' + (index + 1)} Breakdown! Error Code: ${m.alarmCode}`);
                              ...
                              setTimeout(() => {
                                    ...
                                    this.logger.log(`✅ [RECOVER] ${m.name || 'Unit-' + (index + 1)} perbaikan selesai. Cooldown aktif.`);
                              }, recoveryDuration);
                        }
                        ...
                        this.logger.error(`[CRITICAL] Data Mapping Fail - Machine Index ${index}: ${err.message}`);
                  }
            });
      }
